...
import { Chrome, MessageCircle } from 'lucide-react'; // เรียกใช้ไอคอน
...
export default function LoginPage() {
  ...
  const handleLineLogin = async () => {
    await supabase.auth.signInWithOAuth({
      provider: 'auth0', // ใช้ Auth0
      options: { 
        redirectTo: `${window.location.origin}/auth/callback`,
        scopes: 'openid email profile',
        queryParams: {
           connection: 'line' // บังคับเปิด LINE ทันที
        }
      },
    });
  };
  ...
  return (
    <div className="min-h-screen bg-baby-blue/30 flex items-center justify-center p-4">
      <div className="bg-white p-8 rounded-3xl shadow-xl max-w-sm w-full text-center border border-white/50">
        ...
        <h1 className="text-2xl font-bold text-baby-text mb-2">ยินดีต้อนรับสู่ BabyMeal</h1>
        <p className="text-gray-500 mb-8">เข้าสู่ระบบเพื่อบันทึกข้อมูลลูกน้อย<br/>และดูเมนูอาหารสุดพิเศษ</p>
